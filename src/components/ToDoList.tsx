...
const ToDoList = () => {
  ...
  if (empty) {
    return <div className='text-center my-20 text-black-300'>All tasks completed</div>
  }

  return todos?.map(item => <ToDoItem key={item.id} onUpdate={handleUpdate} onToggle={() => handleToggle(item)}
                                      onDelete={handleDelete} todo={item}/>)
}
