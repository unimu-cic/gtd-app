  ...
  return (
    <Dialog.Root open={isOpen}>
      <div className='border mb-4 border-secondary rounded-xl py-4 px-6'>
        ...
        <span
          className={
            classNames('block shrink-0 hover:border-4 duration-300 w-6 h-6 rounded-full cursor-pointer', todo.status ? 'border-green-600 border-[12px]' : 'border-red-500  border-2')
          }
          onClick={(e) => {
            e.preventDefault()
            e.stopPropagation()
        ...
        ></span>
            <h3 className='text-xl font-semibold text-ellipsis overflow-hidden outline-0'>{todo.title}</h3>
          </div>
          <div className='flex items-center gap-1 grow-0'>
            <button
              className={classNames('w-8 h-8 p-1.5 rounded hover:bg-secondary-500/20 duration-300', todo.status ? 'cursor-not-allowed opacity-50' : 'cursor-pointer')}
              onClick={handleEdit}>
              <svg viewBox="0 0 1061 1024">
                <path
                  ...
                  fill="currentColor" p-id="13891"></path>
                <path
            ...
            <button className="w-8 h-8 p-1.5 rounded hover:bg-secondary-500/20 duration-300"
        ...
        {
          todo.description && <div className='text-sm text-secondary-300'>{todo.description}</div>
        }
        {
          todo.enddate && <div className='text-sm text-secondary-300'>{dayjs(todo.enddate).format('YYYY-MM-DD')}</div>
        }
      </div>
      <Dialog.Portal>
          ...
          <form className='flex flex-col gap-4' onSubmit={handleSubmit(onSubmit)}>
            <div className='bg-transparent border-black border rounded-lg grow px-4 py-2'>
              <input defaultValue={todo.title}
                     ...
                     placeholder='update your next task here' {...register("title", {required: 'Your next task can not be empty', minLength: {value: 4, message: 'Password must be at least 4 characters.'}})} />
            </div>
            <div className='bg-transparent border-black border rounded-lg grow px-4 py-2'>
              <input defaultValue={todo.description}
                     className='w-full appearance-none bg-transparent outline-0 placeholder:text-black-300'
                     placeholder='update your description here' {...register("description", {})} />
            </div>
            <div className='bg-transparent border-black border rounded-lg grow px-4 py-2'>
              <input type='date'
                     className='select-none w-full appearance-none bg-transparent outline-0 placeholder:text-black-300'
                     placeholder='update your next task here' {...register("enddate", {})} />
            </div>
            <button type='submit' className='bg-black text-secondary py-2 w-full rounded-lg'>Update</button>
          </form>
        </Dialog.Content>
      </Dialog.Portal>
    </Dialog.Root>
