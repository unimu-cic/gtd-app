    ...
    <div className='my-10'>
      <form className='flex gap-4' onSubmit={handleSubmit(onSubmit)}>
        <div className='bg-black-500 rounded-lg grow px-4 py-2'>
          <input className='w-full appearance-none bg-transparent outline-0 placeholder:text-secondary/50'
                 placeholder='create your next task with enter' {...register("title", {required: 'Your next task can not be empty', minLength: {value: 4, message: 'Password must be at least 4 characters.'}})} />
        </div>
        <button
      ...
      {
        errors.title &&
        <p className='text-xs mt-2 opacity-70 before:content-["*"] before:text-primary'>{errors.title.message}</p>
      }
    </div>
  )
}
